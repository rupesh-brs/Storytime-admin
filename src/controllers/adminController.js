...
const userList = async (req, res, next) => {
  const users = await User.find({ isAdmin: { $ne: true } })
    .populate('languages'); // Populate the languages field
    console.log(users);
  res.render('users', { users: users });
};
...
const categories = async (req, res, next) => {
  ...
  let cat_data = [];
  let categories_with_authors = await Promise.all(
    categories.map(async (e, i) => {
      let data = { ...e._doc };
      const sdata = await fetch(
        `https://api.spotify.com/v1/search?query=Telugu+Tamil+English+Hindi+${e.keywords
          .split(" ")
          .join(
            "+"
          )}&type=show&include_external=audio&market=IN&locale=en-US%2Cen%3Bq%3D0.9&offset=0&limit=50`,
        headers
      );
...
const updateLanguage = async (req, res, next) => {
  try {
    ...
    if (languages.length) {
      return res
        .status(400)
        .json({ message: "Name already exists", success: false });
    }
    const language_code = await Language.find({
      code: code,
      _id: { $ne: req.params.id },
    });
    if (language_code.length) {
      return res
        .status(400)
        .json({ message: "Code already exists", success: false });
    }
    ...
    return res
      .status(400)
      .json({ message: "Something went wrong. Try later", success: false });
  }
};
...
const deleteLanguage = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Language deleted successfully", success: true });
  } catch (error) {
    console.log(error);
    return res
      .status(400)
...
const createLanguages = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Language added successfully", success: true });
  } catch (error) {
    console.log(error);
    return res
      .status(400)
...
const updateCategory = async (req, res, next) => {
  try {
    ...
    const language = await Category.findOneAndUpdate(
      { _id: req.params.id },
...
const getUserLibraries = async (req, res, next) => {
  try {
    ...
    const data = await fetch(
      `https://api.spotify.com/v1/shows?ids=${users.saved_stories}&market=IN`,
      headers
    );
    ...
    if (d.error?.status) {
      return res
        .status(400)
        .json({ message: d.error?.message, success: false });
    }
    return res
      .status(200)
      .json({ data: d, success: true, name: users.first_name });
  } catch (error) {
    console.log(error);
    return res.status(400).json({ message: error, success: false });
  }
    ...
    await Promise.all(
      categories.map(async (category) => {
        ...
        let data = await fetch(
          `https://api.spotify.com/v1/search?q=${keyword}&type=show,episode&market=IN&limit=50&offset=0&include_external=audio`,
          headers
        );
        let author_data = await data.json();
        if (author_data.error) {
          throw author_data.error;
        }
        let total_shows = author_data.shows.total;
        let asdf = await Category.findOneAndUpdate(
          { keywords: category.keywords },
          { count: total_shows, updated_at: new Date() }
        );
      })
    );
    do {
      let data = await fetch(
        `https://api.spotify.com/v1/browse/categories?locale=en-US%2Cen%3Bq%3D0.9&limit=50&offset=${offset}`,
        headers
      );
      let author_data = await data.json();
      if (author_data.error) {
...
const getAuthors = async (headers) => {
  ...
  do {
    let cdata = await fetch(
      `https://api.spotify.com/v1/search?q=${all_cat}&type=show,episode&market=IN&limit=50&offset=${offset}&include_external=audio`,
      headers
    );
  ...
  let all_authors = shows
    .map((e) => e.items.map((i) => i?.publisher).filter((n) => n))
    .flat(3);
  let final_authors = [...new Set(all_authors)];
  await WeeklyCount.findOneAndUpdate(
    {},
    { authors: final_authors.length, updated_at: new Date() },
    { upsert: true }
  );
